...
uint256 hashGenesisBlock("0xde79d5c701a77782b2061ee2319410c52d3b2835f644ccd84d2374b1e19ffd54");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(504365644); // LogiCrypt: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LogiCrypt Signed Message:\n";
...
{
    // LogiCrypt: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 4000000000;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 16500); // LogiCrypt: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 480; // LogiCrypt: 3.5 days
static const int64 nTargetSpacing = 240; // LogiCrypt: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LogiCrypt: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LogiCrypt: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // LogiCrypt: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe752284c9e903d61bed2d7d589d0f361a8c530d5d55c5a29bf6af20ed61da4df");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "FOX News 19DEC2017 Why Trump is smart to not fire Mueller";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 4000000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("7a411ab5d58e6b9ac5d5e5513b2e0eee01a272e044602266e578178835b7b279dc568a98a31c761bd474338b0ff742a912f45c3f2ef1567097ba3cff762cbc6987") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1513717782;
        block.nBits    = 504365644;
        block.nNonce   = 1972599211;

        if (fTestNet)
        {
            block.nTime    = 1513717782;
            block.nNonce   = 765585687;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x522c435e1d328d38e9b1f9aa3945b319af1ef354cf5c6c024c779b9d89f7fb76"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // LogiCrypt: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LogiCryptMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LogiCryptMiner : ProcessBlock, block not accepted");
    }
